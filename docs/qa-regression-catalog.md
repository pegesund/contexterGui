...
| BRIDGE-AX-001 | macOS Accessibility | macOS | An initialized empty AX editor is authoritative and replaces stale context for Notes, TextEdit, Slack, and other AX writing surfaces. | `src/main.rs`: `BridgeManager::try_macos_ax_context`; `src/bridge/ax_mac.rs`: `AxMacBridge::read_context` | `bridge_manager_tests::empty_macos_accessibility_context_replaces_stale_context` |
