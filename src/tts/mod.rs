...
fn build_wav(samples: &[u8]) -> Vec<u8> {
    ...
    // 150ms of silence = 3308 samples = 6616 bytes of zeros
    ...
    wav.extend_from_slice(&vec![0u8; silence_bytes]); // silence padding
    wav.extend_from_slice(samples);
    wav
}

/// Initialize the global TTS on a dedicated thread. Call once at startup.
pub fn init_tts(sdk_dir: &str, voice_name: &str) {
    ...
    std::thread::spawn(move || {
        ...
        let lib = match unsafe { Library::new(&dll_path) } {
            Ok(l) => l,
            Err(e) => {
                eprintln!("Acapela TTS: failed to load {}: {}", dll_path, e);
                let _ = ready_tx.send(false);
                return;
            }
        };

        // Load PlaySoundW from winmm.dll
        let winmm = match unsafe { Library::new("winmm.dll") } {
            Ok(l) => l,
            Err(e) => {
                eprintln!("Acapela TTS: failed to load winmm.dll: {}", e);
                let _ = ready_tx.send(false);
                return;
            }
        };
        let play_sound: PlaySoundFn = unsafe {
            let f: Symbol<PlaySoundFn> = winmm.get(b"PlaySoundW").unwrap();
            *f
        };
        ...
        unsafe {
            ...
            if !fn_init() {
                eprintln!("Acapela TTS: BabTTS_Init failed");
                let _ = ready_tx.send(false);
                return;
            }
            ...
            if engine.is_null() {
                eprintln!("Acapela TTS: BabTTS_Create returned null");
                fn_uninit();
                let _ = ready_tx.send(false);
                return;
            }
            ...
            if err != E_BABTTS_NOERROR {
                eprintln!("Acapela TTS: BabTTS_Open({}) failed: {}", voice_name, err);
                fn_close(engine);
                fn_uninit();
                let _ = ready_tx.send(false);
                return;
            ...
            while let Ok(word) = rx.recv() {
                ...
                // Insert text for synthesis
                let err = fn_insert(engine, text_c.as_ptr(), BABTTS_TXT_UTF8);
                if err != E_BABTTS_NOERROR {
                    eprintln!("TTS InsertText error: {}", err);
                    continue;
                }
                ...
                loop {
                    ...
                    let err = fn_read(engine, pcm_buf.as_mut_ptr(),
                        (pcm_buf.len() / 2) as u32, // size in samples (16-bit = 2 bytes each)
                        &mut generated);
                    if generated > 0 {
                        // generated is count of samples, each sample is 2 bytes (16-bit)
                        all_samples.extend_from_slice(&pcm_buf[..(generated as usize * 2)]);
                    }
                    if err == W_BABTTS_NOMOREDATA || err < 0 {
                        break;
                    }
                }
...
/// Speak a word. Non-blocking — sends to the dedicated TTS thread.
pub fn speak_word(word: &str) {
    if let Some(tx) = TTS_SENDER.get() {
