/// Grammar test runner using SWI-Prolog checker — NYNORSK variant.
/// Runs all test sets in nynorsk/test_data and reports FP/FN.
/// Usage: cargo run --bin test_grammar_swi_nn [set_num ...]
...
fn main() {
    ...
    let test_dir = base.join("../nynorsk/test_data");
    let args: Vec<String> = std::env::args().collect();
    let sets: Vec<usize> = if args.len() > 1 {
        args[1..].iter().filter_map(|a| a.parse().ok()).collect()
    } else {
        // Try sets 1..=39 (we may not have translated all yet)
        (1..=39).collect()
    };
